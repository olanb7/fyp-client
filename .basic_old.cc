...
void Basic::getStats(StationList &l) {
	...
	for (StationList::iterator sta = l.begin(); sta != l.end(); ++sta) {
		...
		debug << "\tMAC: " << sta->mac->unparse().c_str() << "\n\trssi sum per sec = ";
		ewmabug << "hi";	
		...
		if (sta->first_run) {
			sta->ema = sta->ave;
			sta->first_run = 0;
			uint64_t scaled = (static_cast<unsigned int>(sta->ave) << sta->_sec_size.scale());
			sta->_sec_size.assign(scaled);
		}
		else {
			sta->_sec_size.update(sta->ave);
			debug << "\n\n\tEMA: " << k << " x (" << sta->ave << " - " << sta->ema << ") + " << sta->ema;
